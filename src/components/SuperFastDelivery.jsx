...
export default function SuperFastDelivery({ id, className = '' }) {
  ...
  const startAnimation = () => {
    ...
    // render marks/numbers once
    const initializeSpeedometer = () => {
      if (speedMarks.children.length) return; // already init
  ...
  return (
    <section ref={sectionRef} id={id} className={`py-4 sm:py-24 bg-gray-950 flex justify-center ${className}`}>
      <div className="dashboard-wrapper">
        ...
        <p className="text-gray-300 max-w-xl text-center mb-8">{tFast.subtitle}</p>
        <p className="text-gray-400 text-sm text-center italic mt-2">{tFast.note}</p>
        ...
        <div className="odometer-container">
          <div className="odometer-label">{tFast.odometerLabel}</div>
